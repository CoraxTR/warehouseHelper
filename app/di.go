...
func (d *DIContainer) Close() error {
	...
	// 3. Фоновая пометка отгрузок РефГо (ExportOrders → processOrdersShipments):
	// свой ctx от context.Background() и походы в МС через воркерпул. Поэтому
	// остановка до пула: иначе пометки получат ErrPoolStopped, и часть
	// заказов останется помеченной в базе, но не в МС.
	if d.excelExportUC != nil {
		d.excelExportUC.Stop()
	}

	// 4. Воркерпул МС — после всех, кто в него ходит: Submit отдаёт
	// ErrPoolStopped, воркеры дожидаются.
	if d.wp != nil {
		d.wp.Stop()
	}

	// 5. Пул БД — последним: им пользуются все остальные.
	if d.orepo != nil {
		d.orepo.Close()
	}
