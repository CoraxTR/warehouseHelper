...
// внешнего к внутреннему»: сначала те, кто сам что-то качает/пишет, затем
// фоновые задачи модулей, пул БД — последним (им пользуются все).
//
...
func (d *DIContainer) Close() error {
	// Предзагрузка PDF — первой: она пишет файлы через воркерпул МС, и если
	// закрыть пул раньше, докачивающиеся бланки упадут с «клиент остановлен».
	if d.pdfpreloader != nil {
		d.pdfpreloader.StopPreloading()
	}

	// Воркерпул МС: отменяет очередь и ждёт свои горутины. После него любые
	// запросы в МС мгновенно возвращают закрытый канал — фоновые задачи,
	// которые ещё могли бы дёрнуть МС, к этому шагу уже отменены (Shutdown).
	if d.wp != nil {
		d.wp.Stop()
	}

	// Бэкфилл средних продаж: отмена ctx + ожидание горутин. Ждать нужно,
	// чтобы не оборвать идемпотентный upsert на середине батча.
	if d.averageSalesUC != nil {
		d.averageSalesUC.Stop()
	}

	// Пул БД — последним: им пользуются все остальные, после него закрывать
