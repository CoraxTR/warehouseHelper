...
type Cache struct {
	...
	// NoItemCode — у поставщика есть товарное правило без вычитки кода товара
	// (хоть одно; правил нет вовсе — тоже да). Тогда товар для таких сканов
	// задаёт оператор выбором в шапке партии: скан по правилу без кода становится
	// единицей этого товара (вес — из кода, даты сверяются с партией), а скан, чей
	// код известен базе поставщика, в такую партию не принимается.
	NoItemCode bool `json:"no_item_code"`
}
