...
// <id>.<ext> новой схемы и для папки <id>/ старой схемы. Для уменьшенных
// копий (<kind>/<id>.jpg) вернётся не-id — такие значения в DELETE по id не
// совпадают ни с одной строкой БД, что и нужно: строки удаляются по именам
// оригиналов, а копии уходят вместе с файлом оригинала в RemoveAll.
