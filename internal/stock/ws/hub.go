...
// Close закрывает все соединения (остановка приложения).
//
// Под мутексом закрывать НЕЛЬЗЯ: разрыв соединения будит readPump, который
// вызывает Unregister, а тот берёт тот же мутекс — получится взаимная
// блокировка (закрываем под мутексом → ждём Unregister → ждём мутекс).
// Поэтому сначала под мутексом собираем соединения, закрываем их вне его.
//
// Вебсокеты hijack'нуты, поэтому http.Server.Shutdown их не закрывает и не
// ждёт — закрываем сами. Клиент получит onclose и переподключится
// (scheduleReconnect на страницах «Сроки»/«Шорт-лист»); данные при этом не
// теряются — изменения страницы пишут отдельным POST, а не в сокет.
func (h *Hub) Close() {
	h.mu.Lock()
