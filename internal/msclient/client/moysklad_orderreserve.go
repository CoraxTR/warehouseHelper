...
// заменены на массив с обнулённым reserve, а поле state НЕ переносится: статус
// заказа эта операция не меняет (решение владельца 24.09.2026). Идемпотентно
// (повторный вызов после успешного PUT — no-op: резерв уже 0, запрос не
// уходит). Если резерва на позициях нет вовсе — PUT не отправляется (менять
// нечего).
...
func zeroOrderReserves(raw json.RawMessage) (out json.RawMessage, changed bool, err error) {
	...
	// Статус в PUT НЕ переносим (решение владельца 24.09.2026): тело — эхо GET,
	// и без этого PUT заново утверждал бы статус из эха, затирая изменения,
	// сделанные после чтения (в частности расформирование заказа целиком —
	// «Отменен» — и возврат заказа в работу). Статус меняют только шаги подбора.
	delete(body, "state")
