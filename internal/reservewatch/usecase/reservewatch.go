...
func (uc *UseCase) collectProblems(ctx context.Context, orders []client.MSOrder) (map[string]map[reservewatch.Kind][]problemItem, error) {
	...
	for _, ref := range refs {
		...
		// Заглушка подбора — не проблема резерва (фильтр по СЫРОМУ
		// количеству, до перевода в единицы сверки: 0,0002 кг в граммах
		// округлилось бы до нуля и увело за собой реальный лёгкий кусок).
		if ref.pos.Quantity < stubQtyMax {
			continue
		}
...
// stubQtyMax — верхняя граница количества строки-заглушки подбора
// («ожидание единицы»: msorders ставит quantity 0,0001, reserve 0 — минимум,
// который принимает МС). Строки ниже порога — не проблема резерва: единицы
// в заказе фактически нет, откладывать нечего; уведомление «нужно отложить»
// по ней было бы требованием отложить несуществующий кусок.
//
// Порог 0,0002 (решение владельца 14.09) вместо прежнего отсечения по
// округлению (q == 0 в граммах/штуках): куски легче 0,5 кг реальны, и
// строка 0,15 кг обязана сверяться, а не считаться пустой.
const stubQtyMax = 0.0002
