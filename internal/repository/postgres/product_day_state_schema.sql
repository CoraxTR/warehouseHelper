...
-- скидка среди лотов (канал general: COALESCE(manual, NULLIF(plain,0)) — ручная,
-- включая заданный 0, перекрывает plain; ноль в plain-колонке = «скидки нет»), discount = то же
-- значение на старте. Лоты, накрытые запретом менеджера (ручная 0 на паре с не меньшим
-- сроком), в скидку дня дают 0.
