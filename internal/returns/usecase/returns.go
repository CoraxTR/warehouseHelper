...
func (uc *UseCase) tick(ctx context.Context) error {
	...
	// Окно долистано: курсор = край окна + 1 секунда. +1с обязателен: фильтр
	// листа идёт секундами (time.DateTime, мс в фильтр не попадают), и событие
	// с моментом == курсор-секунды (например, 10:16:01.637 при фильтре
	// >=10:16:01) навсегда оставалось бы в окне и перечитывалось каждый тик
	// (баг прода 09.09: те же события в логе каждую минуту). События краевой
	// секунды уже обработаны — курсор за секунду их выталкивает.
	// Не долистали (бюджет) — курсор не трогаем: следующий тик продолжит
	// с того же since.
	if done && !edge.IsZero() {
		if err := uc.repo.SetCursor(ctx, edge.Add(time.Second)); err != nil {
